...
| `payload.hasActiveTab` | `boolean` | Optional: Sent during HEARTBEAT to inform host election |
...
│       ├── core/
│       │   └── signaling.py             # WebSocket handler, validation & routing
│       ├── managers/
...
|---|---|---|
| `Peer.__init__` | `(peer_id: str, websocket: WebSocket)` | Sets `self.id`, `self.websocket`, `self.lamport_clock = None`, `self.has_active_tab = False`, `self.last_heartbeat = time.time()` |
| `Peer.update_state` | `(clock: int, has_tab: bool) → None` | Syncs internal clock and active tab status based on incoming messages |
...
| `Room.elect_new_host` | `() → Optional[str]` | Sorts remaining peers by `(has_active_tab=True, lamport_clock)` **descending**. Picks the most up-to-date peer with an active video tab. **Returns** new host ID or `None` if empty |

---

### `app/managers/connection_manager.py`
...
| `process_message` | `async (room, sender_id, message) → None` | 1) **Security:** Clamps `videoTime` to ≥ 0. Rejects malicious `lamportClock` jumps (>100 delta or negative). If initial clock not set, sets it to the first message's clock. 2) **Routing:** Sync commands → broadcast to room. WebRTC signaling → specific peer. 3) **State:** Updates Peer clock & tab state on HEARTBEAT. |
...
### `SyncEngine.js` — class `SyncEngine`
...
| `_startHeartbeat` | `() → void` | Every 5s: Host queries tab → broadcasts HEARTBEAT (includes `hasActiveTab: true/false`). Non-host sends PING. Disconnects host if 0 YouTube tabs |
...
### `ConnectionManager.js` — class `ConnectionManager`
...
### `InterruptionModule.js`

Strategy Pattern for ad/buffering handling.

**Strategies:**

| Class | `handleStart` | `handleEnd` | Description |
|---|---|---|---|
| `TimeBoxedPausePolicy` | Pauses room. Starts 10s timer. | Resumes room, clears timer. | **(Default)** Pauses room for buffering, but if user is stuck for > 10s, auto-resumes room and flags user as 'ignored' until they catch up. |
| `PauseAllPolicy` | Pauses room for everyone. | Resumes room if no users still interrupted. | Strict sync. Susceptible to buffering deadlocks. |
| `IgnorePolicy` | No-op | No-op | Interrupted user catches up normally on their own. |

**`InterruptionModule` class:**

| Method | Signature | I/O |
|---|---|---|
| `constructor` | `(syncEngine, connectionManager)` | Default policy: `TimeBoxedPausePolicy` |
| `setPolicy` | `(policyName: string) → void` | Switch policies dynamically |
...
|---|---|---|
| `PlayCommand` | Seeks (if videoTime set), then plays | `{videoTime, videoId}` |
| `PauseCommand` | Pauses, then seeks (if videoTime set) | `{videoTime, videoId}` |
| `SeekCommand` | Seeks to `payload.videoTime` | `{videoTime, videoId}` |
| `ForceSyncCommand` | Seeks to `payload.videoTime`, then plays | `{videoTime, videoId}` |
| `RoomStateCommand` | Seeks (if videoTime), then play/pause based on `isPaused` | `{videoId, videoTime, isPaused}` |
| `RequestStateCommand` | No-op (intercepted by SyncEngine) | `{}` |
...
### `YouTubeAdapter.js` — Concrete Implementation
...
| **Strategy** | `InterruptionPolicy` → `TimeBoxedPausePolicy` / `PauseAllPolicy` / `IgnorePolicy` | Swappable interruption handling |
...
sequenceDiagram
    ...
    Server->>Server: room.elect_new_host() → highest lamport_clock + active tab
